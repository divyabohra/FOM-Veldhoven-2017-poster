...
\begin{tikzpicture}[x=0.5\textwidth]
    ...
    \node[inner sep=0,text width=0.8\textwidth,text centered,font=\Huge] (Title) at (-0.3,0) 
    {
        ...
        {\sffamily\Huge \underline{Mandar Chandorkar}$^1$, Enrico Camporeale$^1$, Simon Wing$^2$}\\
        \vspace{-0.2\baselineskip}
        {\sffamily\LARGE 1 Multiscale Dynamics, CWI, Amsterdam, 2 Johns Hopkins University}\\
...
The Dst index is an index of magnetic activity derived from a network of near-equatorial geomagnetic observatories. Hourly records of Dst are available since 1957. Dst has hourly frequency and and 1 nT resolution.

%\vspace{2\baselineskip}
\mysection{State of the Art}

\begin{enumerate}
    \item Nonlinear Auto Regressive with exogenous Inputs (solar wind speed, IMF $B_z$, etc) \emph{NARX/NARMAX}
    \item Recurrent Neural Networks
    \item Approximate physical models based on ODE: $\frac{d Dst(t)}{dt} = Q(t) - \frac{Dst(t)}{\tau}$
    \item $\hat{Dst}(t) = Dst(t-1)$: the (in)famous \textit{Persistence} model.
\end{enumerate}
%\vspace{2\baselineskip}

One Step Ahead $Dst$ prediction models compared in Ji et al. (J. Geophys. Res. 2012) over 63 storms (storm-time only)
\vspace{\baselineskip}
...
\hline
\noalign{\smallskip}
TL & Temerin and Li (2002) & Auto regressive model decomposable\\&& into additive terms.     \\
NM & Boynton et al. (2011) & Non linear auto regressive with\\ && exogenous inputs. \\
B & Burton et al. (1975) & Prediction of $Dst$ by solving\\ && ODE having injection and decay terms. \\
W & Wang et al. (2003) & Obtained by modification\\ && of injection term in Burton. \\
FL & Fenrich and Luhmann (1998) & Uses polarity of magnetic clouds\\ && to predict geomagnetic response.\\
OM & O'Brien et al. (2000) & Modification of injection\\ && term in Burton.\\
\noalign{\smallskip}
\hline
\end{tabular}
...
Using the solar wind speed and $z$ component of IMF as predictive variables, we train a Gaussian Process model to predict one-hour-ahead the Dst index (GP-ARX). We compare the performance of Gaussian Process models with the current state of the art in Dst prediction.

%\vspace{\baselineskip}
%\includegraphics[width=.9\hsize]{Compare_RMSE}
\myfig{Compare_RMSE}{0.8}

\myfig{Compare_CC}{0.8}
%\includegraphics[width=.9\hsize]{Compare_CC}
\label{fig:rmse}
\mycaption{Performance of our models (GP-ARX and GP-AR) compared with previous models: Root Mean Square Error and Mean Correlation Coefficient} 
\center
%\includegraphics[width=.8\hsize]{Compare_RelProb}\mycaption{Empirical Cumulative Probability of Relative Errors.}
\myfig{Compare_RelProb}{0.8}
\mycaption{Empirical Cumulative Probability of Relative Errors.}

%\vspace{\baselineskip}
\mysection{Conclusions}
\begin{enumerate}
    \item \emph{Persistence} model must be central in the model evaluation process in the context of \emph{one step ahead} prediction of the $Dst$ index.
    \item \emph{Gaussian Process} AR and ARX models give encouraging benefits in One-Step-Ahead prediction even when compared to the \emph{Persistence} model.
    \item More informative performance metrics for evaluation and comparison are required.
